...
# ── NO_PROXY только для текущей сессии деплоя (не затрагивает систему) ──────────────────────
NO_PROXY_LIST="localhost,127.0.0.1,10.1.5.97,10.1.5.6,10.1.26.2,chat.ehd-zr.cbr.ru"
export NO_PROXY="\${NO_PROXY_LIST}"
export no_proxy="\${NO_PROXY_LIST}"
ok "NO_PROXY установлен для сессии: \${NO_PROXY_LIST}"
...
# ── Проверка MCP-серверов (через curl --noproxy) ───────────────────────────────────────────────────
